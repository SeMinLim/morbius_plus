...
const char AMINO_ACID_TABLE[AMINO_ACID_NUM + 1] = "ACDEFGHIKLMNPQRSTVWY";


// Generate a uniformly random protein background
void generateProteinBackground( mt19937_64 &randomGenerator, string &sequence ) {
	uniform_int_distribution<int> aminoAcidDistribution(0, AMINO_ACID_NUM - 1);
	sequence.resize(PROTEIN_SEQUENCE_LENGTH);
	for ( size_t position = 0; position < PROTEIN_SEQUENCE_LENGTH; position ++ ) {
		sequence[position] = AMINO_ACID_TABLE[aminoAcidDistribution(randomGenerator)];
	}
}
...
void generateDatasetPair( const Config *config,
	...
	proteinGroundTruthOutput << "sequence_id\tbinding_site_id\timplanted_offset\timplanted_length\timplanted_sequence\n";
	...
	for ( uint64_t sequenceID = 0; sequenceID < spec->sequenceNum; sequenceID ++ ) {
		...
		const string &proteinMotif = proteinMotifs[bindingSiteID];
		...
		// Generate the protein sequence and record the exact implanted position
		string proteinSequence;
		generateProteinBackground(randomGenerator, proteinSequence);
		uniform_int_distribution<uint64_t> proteinOffsetDistribution(
			0,
			PROTEIN_SEQUENCE_LENGTH - proteinMotif.size()
		);
		uint64_t proteinOffset = proteinOffsetDistribution(randomGenerator);
		proteinSequence.replace(proteinOffset, proteinMotif.size(), proteinMotif);
		if ( proteinSequence.substr(proteinOffset, proteinMotif.size()) != proteinMotif ) {
			printf( "Protein implantation validation failed for dataset %d, sequence %lu.\n",
				spec->datasetID,
				(unsigned long)sequenceID );
			fflush( stdout );
			exit(1);
