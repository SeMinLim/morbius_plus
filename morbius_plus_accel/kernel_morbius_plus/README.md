...
The kernel uses the direct `HOST[0]` memory connection from `kernel_example_add_host` and retains four independent Gibbs pipeline states on chip.
...
- `NumPE_Profiler = 8`
- `NumPE_LPM = 4`
- physical dual-mode PWL lanes: `max(NumPE_Profiler, NumPE_LPM) = 8`
...
make all TARGET=hw
make run TARGET=hw MORBIUS_ARGS="--input /path/to/input.fa --output output/result --alphabet dna --motif-length 16 --pipelines 4 --max-updates 32768 --score-threshold 1.0 --seed 1"
```
