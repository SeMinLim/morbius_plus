...
static uint32_t readUInt32Model( const uint8_t *buffer ) {
	uint32_t value = 0;
	for ( int i = 0; i < 4; i ++ ) {
		value = value | ((uint32_t)buffer[i] << (8 * i));
	}
	return value;
}
...
static void writeUInt32Model( uint8_t *buffer, uint32_t value ) {
	for ( int i = 0; i < 4; i ++ ) {
		buffer[i] = (uint8_t)((value >> (8 * i)) & 0xffU);
	}
}


static uint32_t getBitsModel( const uint8_t *buffer,
			      size_t bitOffset,
			      size_t bitWidth ) {
	uint32_t value = 0;
	for ( size_t bitIdx = 0; bitIdx < bitWidth; bitIdx ++ ) {
		...
		uint32_t bitValue =
			(uint32_t)((buffer[inputBit >> 3] >> (inputBit & 0x7U)) & 0x1U);
...
static void setBitsModel( uint8_t *buffer,
			  size_t bitOffset,
			  uint32_t value,
			  size_t bitWidth ) {
	for ( size_t bitIdx = 0; bitIdx < bitWidth; bitIdx ++ ) {
		...
		if ( ((value >> bitIdx) & 0x1U) != 0 ) {
			buffer[outputBit >> 3] |=
				(uint8_t)(1U << (outputBit & 0x7U));
		}
	}
}


static uint8_t sequenceSymbol( const ModelPipelineState *state,
			       uint32_t position ) {
...
static uint64_t alignMassModel( uint64_t mass, uint32_t exponentDifference ) {
	if ( exponentDifference >= 36 ) return 0;
	return mass >> exponentDifference;
}
...
static uint32_t sampleCandidateModel( ModelPipelineState *state ) {
	...
	for ( uint32_t segmentStart = 0;
		...
		uint32_t validNum =
			min((uint32_t)ACCELSEGMENTSIZE, candidateNum - segmentStart);
		...
		for ( uint32_t localIdx = 0; localIdx < validNum; localIdx ++ ) {
			...
			for ( uint32_t column = 0;
			      column < state->motifLength;
			      column ++ ) {
				uint32_t symbol = sequenceSymbol(
					state,
					segmentStart + localIdx + column
				);
				value = value +
					state->lpmQ12[matrixIndex(state, symbol, column)];
			}
		...
		for ( uint32_t localIdx = 0;
		      localIdx < ACCELSEGMENTSIZE;
		      localIdx ++ ) {
			if ( localIdx < validNum ) {
				uint32_t difference = exponentQ12 - logProb[localIdx];
		...
		uint32_t localRandom =
			randomWord(&state->randomGenerator) >> (32 - RANDOMFRACTIONBITS);
		uint32_t reservoirRandom =
			randomWord(&state->randomGenerator) >> (32 - RANDOMFRACTIONBITS);
		uint32_t localOffset = selectLocalCandidateModel(
			weight.data(),
			segmentMass,
			localRandom,
			validNum
		...
		uint32_t nextExponent = globalMassValid ?
			max(globalExponent, exponent) : exponent;
		uint64_t oldMass = globalMassValid ?
			alignMassModel(globalMass, nextExponent - globalExponent) : 0;
		uint64_t newMass =
			alignMassModel(segmentMassQ24, nextExponent - exponent);
		uint64_t totalMass = oldMass + newMass;
		uint64_t replacementThreshold =
			(totalMass * (uint64_t)reservoirRandom) >> RANDOMFRACTIONBITS;

		if ( globalMassValid == false || replacementThreshold < newMass ) {
			selectedOffset = segmentStart + localOffset;
		}
		globalMassValid = true;
		globalExponent = nextExponent;
		globalMass = totalMass;
	}
...
static AccelWireResult runPipelineModel( ModelPipelineState *state,
	...
	if ( bootstrap == false ) {
		for ( uint32_t column = 0; column < state->motifLength; column ++ ) {
			uint32_t currentSymbol =
				sequenceSymbol(state, tentativeOffset + column);
			...
			if ( previousSymbol != currentSymbol ) {
				uint32_t previousIdx =
					matrixIndex(state, previousSymbol, column);
				state->lpmQ12[previousIdx] =
					calculateLog2PWLQ12(state->bpm[previousIdx]);
				state->lpmQ12[currentIdx] =
					calculateLog2PWLQ12(state->bpm[currentIdx]);
			}
		}
	}
	...
	for ( uint32_t column = 0; column < state->motifLength; column ++ ) {
		...
		for ( uint32_t candidateSymbol = 0;
		      candidateSymbol < state->alphabetSize;
		      candidateSymbol ++ ) {
			columnMaximum = max(
				columnMaximum,
				state->bpm[matrixIndex(state, candidateSymbol, column)]
			);
		}
...
static void loadSequenceModel( ModelPipelineState *state,
	...
	state->sequence.assign(
		input,
		input + (size_t)sequenceBeatNum * ACCELBEATBYTES
	);
...
static void packResultModel( uint8_t *resultGroup,
	...
	for ( int beatIdx = 0; beatIdx < ACCELRESULTBEATNUM; beatIdx ++ ) {
		...
		for ( int slotIdx = 0;
		      slotIdx < ACCELPIPELINEPERRESULTBEAT;
		      slotIdx ++ ) {
			...
			setBitsModel(word,
				     base + 11,
				     result[pipelineIdx].bestUpdate ? 1 : 0,
				     1);
			setBitsModel(word,
				     base + 12,
				     result[pipelineIdx].terminated ? 1 : 0,
				     1);
			setBitsModel(word, base + 13, 1, 1);
...
int executeModelKernel( const vector<uint8_t> &input,
	...
	if ( pipelineNum != ACCELMAXPIPELINE ||
	     sequenceLength < motifLength ||
	     sequenceLength > ACCELMAXSEQUENCELENGTH ||
	     motifLength == 0 || motifLength > ACCELMAXMOTIFLENGTH ||
	     alphabetSize == 0 || alphabetSize > ACCELALPHABETMAX ||
	     sequenceBeatNum !=
		(sequenceLength + ACCELBEATBYTES - 1) / ACCELBEATBYTES ) {
	...
	if ( command == ACCELCOMMAND_BOOTSTRAP ) {
		resetModelKernel();
		if ( cursor + (size_t)sequenceBeatNum * ACCELBEATBYTES > input.size() ) {
			return 1;
		}
		...
		for ( int pipelineIdx = 0;
		      pipelineIdx < ACCELMAXPIPELINE;
		      pipelineIdx ++ ) {
			if ( cursor + ACCELBEATBYTES +
			     (size_t)motifLength * 2 * ACCELBEATBYTES > input.size() ) {
				return 1;
			}
			...
			for ( int stateIdx = 0; stateIdx < 4; stateIdx ++ ) {
				state.randomGenerator.state[stateIdx] =
					readUInt32Model(input.data() + cursor + stateIdx * 4);
			}
		...
		for ( int pipelineIdx = 0;
		      pipelineIdx < ACCELMAXPIPELINE;
		      pipelineIdx ++ ) {
			result[pipelineIdx] = runPipelineModel(
				&modelKernelState.pipeline[pipelineIdx],
				0,
				true
			);
		}
		processedNum = 1;
		packResultModel(output.data(), result);
		...
		for ( uint32_t itemIdx = 0; itemIdx < batchSize; itemIdx ++ ) {
			if ( cursor + (size_t)sequenceBeatNum * ACCELBEATBYTES +
			     ACCELBEATBYTES > input.size() ) {
				return 1;
			}
			const uint8_t *sequenceInput = input.data() + cursor;
			cursor += (size_t)sequenceBeatNum * ACCELBEATBYTES;
			...
			for ( int pipelineIdx = 0;
				...
				uint32_t tentativeOffset =
					readUInt32Model(offsetWord + pipelineIdx * 4);
				result[pipelineIdx] = runPipelineModel(
					&state,
					tentativeOffset,
					false
			...
			packResultModel(
				output.data() + (size_t)itemIdx * ACCELRESULTBYTES,
				result
			);
			processedNum ++;
		}
	...
	packSummaryModel(
		output.data() + (size_t)batchSize * ACCELRESULTBYTES,
		processedNum,
		allDone
	);
	return 0;
